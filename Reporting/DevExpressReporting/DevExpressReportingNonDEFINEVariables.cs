...
namespace WpfCSCS.Reporting.DevExpressReportingNonDEFINEVariables
{
    ...
    public class ReportFunction : ParserFunction
    {
        ...
        // Resolve a variable name first from the interpreter (implicit variables), then fallback to GUI.DEFINES.
        private bool TryResolveVar(string name, out Variable var)
        {
            var = Variable.EmptyInstance;
            string key = name?.ToLower();
            if (Script?.InterpreterInstance != null && !string.IsNullOrWhiteSpace(key))
            {
                var fromInterpreter = Script.InterpreterInstance.GetVariableValue(key);
                if (fromInterpreter != null && fromInterpreter != Variable.EmptyInstance && fromInterpreter.Type != Variable.VarType.NONE)
                {
                    var = fromInterpreter;
                    return true;
                }
            }
            if (!string.IsNullOrWhiteSpace(key) && Gui != null && Gui.DEFINES.TryGetValue(key, out DefineVariable def))
            {
                var = def;
                return true;
            }
        ...
        private Variable SetupReport()
        {
            ...
            if (isMainReport)
            {
                ...
                foreach (var fieldName in fieldsOfReports[1])
                {
                    ...
                    if (TryResolveVar(fieldName, out Variable defVar))
                    {
                        switch (defVar.Type)
                        {
                ...
                foreach (var picture in allPictures)
                {
                    ...
                    if (!string.IsNullOrEmpty(picture.Tag.ToString()))
                    {
                        ...
                        if (TryResolveVar(imageSourceVariableName, out Variable defVar))
                        {
        ...
        private void OutputReport()
        {
            ...
            for (int i = 0; i < fieldsOfReports[reportHndlNum].Count; i++)
            {
                ...
                else if (chartTags.Any(p => p == dataTableFieldName))
                {
                    ...
                    if (TryResolveVar(dataTableFieldName + "_val_1", out Variable defVarVal1))
                    {
                        List<string> chartArgs = new List<string>();
                        if (TryResolveVar(dataTableFieldName + "_args", out Variable defVarArgs))
                        {
                            chartArgs = defVarArgs.Tuple.Select(p => p.AsString()).ToList();
                        }

                        if (TryResolveVar(dataTableFieldName + "_val_2", out Variable defVarVal2))
                        {
                            dt.Columns.Add(new DataColumn("Value2", typeof(double)));
                            //numberOfSeries = 2;

                            if (TryResolveVar(dataTableFieldName + "_val_3", out Variable defVarVal3))
                            {
                                ...
                                for (int j = 0; j < defVarVal1.Tuple.Count; j++)
                                {
                                    ...
                                    row["Value1"] = defVarVal1.Tuple[j].AsDouble();
                                    row["Value2"] = defVarVal2.Tuple[j].AsDouble();
                                    row["Value3"] = defVarVal3.Tuple[j].AsDouble();
                                    dt.Rows.Add(row);
                                }
                            }
                            else
                ...
                else if (TryResolveVar(dataTableFieldName, out Variable defVar))
                {
                    switch (defVar.Type)
                    {
                        case Variable.VarType.STRING:
                        ...
                        case Variable.VarType.NUMBER:
                            ...
                            break;
                        case Variable.VarType.DATETIME:
                            newObjectArray[i] = defVar.AsDateTime();
                            break;
                        default:
                            break;
                    }
            ...
            foreach (var band in allBands)
            {
                ...
                foreach (var control in allControls)
                {
                    if (!control.Name.StartsWith("xr"))
                    {
                        ...
                        if (TryResolveVar(controlName, out Variable defVar)) // visibility
                        {
                        ...
                        if (TryResolveVar(controlName + "cl", out Variable defVar1)) // font color
                        {
                            var label = control as XRLabel;
                            label.ForeColor = System.Drawing.Color.FromName(defVar1.AsString());
                        }
                        if (TryResolveVar(controlName + "lt", out Variable defVar2)) // left margin
                        {
                            control.LeftF = (float)defVar2.AsDouble();
                        }
                        if (TryResolveVar(controlName + "top", out Variable defVar3)) // left margin
                        {
                            control.TopF = (float)defVar3.AsDouble();
                        }
                        if (TryResolveVar(controlName + "fc", out Variable defVar4)) // font color
                        {
                            try
                            {
                        ...
                        if (TryResolveVar(controlName + "fs", out Variable defVar5)) // font size
                        {
                        ...
                        if (TryResolveVar(controlName + "fn", out Variable defVar6)) // font name
                        {
                            System.Drawing.Font oldFont = control.GetEffectiveFont();
                        ...
                        if (TryResolveVar(control.Name.ToLower() + "wd", out Variable defVar7))
                        {
                            var requestedWidth = (float)defVar7.AsDouble();
                            if (requestedWidth != 0)
                                control.WidthF = requestedWidth;
                        }
                        if (TryResolveVar(control.Name.ToLower() + "ht", out Variable defVar8))
                        {
        ...
        private void UpdateReport()
        {
            ...
            {
                if (TryResolveVar(variable.ToLower(), out Variable defVar))
                {
                    switch (defVar.Type)
                    {
                        case Variable.VarType.STRING:
